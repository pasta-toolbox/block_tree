...
//        manipulate lpf_ptr
        ...
        size_type total_cmprs = 0;
        size_type total_blocks = 0;
        size_type hits = 0;
        size_type misses = 0;
        size_type new_blocks = 0;
...
//            std::cout << "Blocksize: " << block_size << " Blocks in Level: " << block_text_inx.size();
            ...
            for(int block_in_lvl = block_text_inx.size() - 1; block_in_lvl >= 0; block_in_lvl--) {
                ...
                size_type initial_index = block_text_inx[block_in_lvl];
                size_type ind = initial_index;
...
//                        if (initial_index == 410) {
//                            std::cout << std::endl << initial_index << " " << b << std::endl;
//                            for (auto z = 0; z < block_size; z++) {
//                                std::cout << z << " " << static_cast<int>(text[initial_index]) << " " << static_cast<int>(text[ptr]) << std::endl;
//                            }
//                        }
                        if (has_ptr) {
...
//            }
            std::vector<size_type> block_text_inx_new(0,0);
            block_size = block_size / this->tau_;
            for (int i = 0; i < bv->size(); i++) {
                if ((*bv)[i] == 1) {
                    for (int j = 0; j < this->tau_; j++) {
            ...
            int ptr_size = (8 * sizeof(max_pointer)) - this->leading_zeros(max_pointer);
            int off_size = (8 * sizeof(max_offset)) - this->leading_zeros(max_offset);
            ...
            std::vector<std::vector<size_type>> g_temp(this->u_chars_);
            for(int j = 0; j < pointers.size(); j++) {
                auto pointer = pointers[pointers.size() - 1 - j];
                auto offset = offsets[pointers.size() - 1 - j];
                (*p)[j] = pointer;
                (*o)[j] = offset;
//                std::vector<size_type> g_counter(this->u_chars_);
//                for (int i = 0; i < offset; i++) {
//                    g_counter[this->chars_index_[text[block_text_inx[pointer] + i]]]++;
//                }
...
//        std::cout << this->leaves_.size() << " so much space" << std::endl;
    ...
    int32_t print_bv(int i) {
        for (int j = 0; j < this->block_per_lvl_[i]; j++) {
            std::cout << (*this->block_tree_types_[i])[j];
        }
        std::cout << std::endl;
        return 0;
    }
    int32_t print_leafs() {
        for (int j = 0; j < this->leaves_.size(); j++) {
            std::cout << this->leaves_[j];
        }
        std::cout << std::endl;
        return 0;
    }
