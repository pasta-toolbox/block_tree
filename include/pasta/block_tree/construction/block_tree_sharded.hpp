...
#  endif
      ...
      prune(levels);
#ifdef BT_INSTRUMENT
      size_t prune_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(
                            Clock::now() - now)
                            .count();
      now = Clock::now();
#  ifdef BT_DBG
...
#ifdef BT_INSTRUMENT
      size_t make_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(
                           Clock::now() - now)
                           .count();
#  ifdef BT_DBG
...
#  endif
    ...
    [[maybe_unused]] static void print_aggregate(
        const char* name,
    ...
    /// @brief Scan through the blocks pairwise in order to identify which
    /// blocks should be replaced with back blocks.
    ///
    /// @param text The input string.
    /// @param level The data for the current level.
    /// @param is_padded `true` iff the last block on this level *does not* end
    /// at
    ///   the exact end of the text.
    /// @param threads Number of threads to use
    /// @param queue_size The size of the queue to use per thread in the sharded
    ///   hash map.
    /// @tparam use_hash Whether to use a Rabin-Karp hasher to hash substrings
    /// or
...
#ifdef BT_INSTRUMENT
      bp_scan_pairs_ns += std::chrono::duration_cast<std::chrono::nanoseconds>(
                              Clock::now() - now)
                              .count();
...
#  endif
...
#ifdef BT_INSTRUMENT
      bp_markings_ns += std::chrono::duration_cast<std::chrono::nanoseconds>(
                            Clock::now() - now)
                            .count();
      now = Clock::now();
#endif
...
#ifdef BT_INSTRUMENT
      bp_bitvec_ns += std::chrono::duration_cast<std::chrono::nanoseconds>(
                          Clock::now() - now)
                          .count();
#endif
    }
    ...
    static inline void scan_windows_in_block_pair_identity(
        const std::vector<input_type>& text,
    ...
    /// text windows or to use the block's content as a hash. For any window
    /// size greater than 8 bytes, use Rabin-Karp.
    template <UseHash use_hash = UseHash::RABIN_KARP>
...
#  pragma omp parallel default(none) num_threads(threads)                      \
...
#endif
      {
        ...
        if constexpr (use_hash == UseHash::RABIN_KARP) {
          ...
          const uint64_t HASH_MASK =
              internal::sharded::HASH_MASKS[block_size / sizeof(input_type)];
          for (size_t i = start; i < end; ++i) {
            const size_t block_start = block_starts[i];
            const input_type* block_start_ptr = text.data() + block_start;
            const uint64_t hash_value =
...
#ifdef BT_INSTRUMENT
      b_scan_blocks_ns += std::chrono::duration_cast<std::chrono::nanoseconds>(
                              Clock::now() - now)
                              .count();
      now = Clock::now();
...
#  endif
    ...
    static void scan_windows_in_block_identity(
        const std::vector<input_type>& text,
        const size_t block_start,
        BlockMap& links,
        LevelData& level_data,
    ...
    [[nodiscard]] LevelData generate_next_level(
        const std::vector<input_type>& text,
    ...
    void make_tree(const std::vector<input_type>& text,
      ...
      for (size_t block = 0; block < last_is_internal.size(); block++) {
        ...
        for (size_t b = 0;
             b < static_cast<size_t>(this->leaf_size * this->tau_);
             b++) {
    ...
    bool prune_block(std::vector<LevelData> & levels,
      ...
      if (level_index < levels.size() - 1) {
        const size_type last_child = std::min<size_type>(
            first_child + this->tau_ - 1,
