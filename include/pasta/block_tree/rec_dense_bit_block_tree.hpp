...
public:
  ...
  bool access(const size_type bit_index) const {
    ...
    int64_t block_index = bit_index / block_size;
    int64_t off = bit_index % block_size;
    ...
    return (*leaf_bits_)[block_index * leaf_size + off];
  };
...
public:
  ...
  rank1(const size_type bit_index) const {
    ...
    const auto& top_offsets = *block_tree_offsets_[0];
    size_t block_size = block_size_lvl_[0];
    size_t block_index = bit_index / block_size;
    size_t block_offset = bit_index % block_size;
    ...
    for (size_t block = 0; block < child * leaf_size; block++) {
      rank += (*leaf_bits_)[prefix_leaves * leaf_size + block];
    }
    for (size_t block = 0; block < block_offset; block++) {
      rank += (*leaf_bits_)[block_index * leaf_size + block];
    }
    return rank;
  }
...
protected:
  ...
  size_type bit_rank_leaf(size_type leaf_index, size_type max_char_index) {
    if (static_cast<uint64_t>(leaf_index * leaf_size) >= leaf_bits_->size()) {
      return 0;
    }

    size_type result = 0;
    for (size_type i = 0; i < max_char_index; ++i) {
      result += (*leaf_bits_)[leaf_index * leaf_size + i];
    }
    return result;
  }
};
...
using DenseBitBlockTree = RecursiveDenseBitBlockTree<size_type>;
