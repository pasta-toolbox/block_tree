...
int main(int argc, char *argv[]) {
  ...
  //    std::ifstream
  //    t("/home/daniel/blocktree-experiments/data/einstein.de.txt");
  std::ifstream t("/home/daniel/blocktree-experiments/data/einstein.en.txt");
  //    std::ifstream
  //    t("/home/daniel/blocktree-experiments/data/world_leaders");
  ...
  //    BV_BlockTree_fp_theory<uint8_t, int32_t> *fp_bt = new
  //    BV_BlockTree_fp_theory<uint8_t, int32_t>(vec,2,1,1);

  //    BV_BlockTree_fp_pruned<uint8_t, int32_t> *fp_bt = new
  //    BV_BlockTree_fp_pruned<uint8_t, int32_t>(vec, 16, 4, 1);
  ...
  auto ms_intstack =
      std::chrono::duration_cast<std::chrono::milliseconds>(t0b - t0a);
  ...
  //    BV_BlockTree_lpf_heuristic<uint8_t, int32_t>* fp_bt = new
  //    BV_BlockTree_lpf_heuristic<uint8_t, int32_t>(vec, 2, 1);
  //    BV_BlockTree_lpf_theory<uint8_t, int32_t>* fp_bt = new
  //    BV_BlockTree_lpf_theory<uint8_t, int32_t>(vec, 2, 1,1,lpfArray, prevOcc,
  //    lzPhrases);
  auto t03 = std::chrono::high_resolution_clock::now();
  // auto* fpTheory_bt = new BV_BlockTree_fp_theory<uint8_t, int32_t>(vec, tau,
  // mls,s,256, CUT_FIRST_LEVELS);
  auto t04 = std::chrono::high_resolution_clock::now();
  auto ms_int_fp_theo =
      std::chrono::duration_cast<std::chrono::milliseconds>(t04 - t03);
  std::cout << "FP THEORY TIME " << ms_int_fp_theo.count() << std::endl;
  auto *fpPruned_bt = new BlockTreeFP<uint8_t, int32_t>(
      vec, tau, mls, s, 256, CUT_FIRST_LEVELS, EXTENDED_PRUNE);
  auto t05 = std::chrono::high_resolution_clock::now();
  auto ms_int_fp_prune =
      std::chrono::duration_cast<std::chrono::milliseconds>(t05 - t04);
  std::cout << "FP PRUNE TIME " << ms_int_fp_prune.count() << std::endl;
  auto t0x = std::chrono::high_resolution_clock::now();
  auto *fpPruned_simple_bt = new BlockTreeFP<uint8_t, int32_t>(
      vec, tau, mls, s, 256, CUT_FIRST_LEVELS, SIMPLE_PRUNE);
  auto t0y = std::chrono::high_resolution_clock::now();
  auto ms_int_fp_prune_simple =
      std::chrono::duration_cast<std::chrono::milliseconds>(t0y - t0x);
  std::cout << "FP PRUNE SIMPLE TIME " << ms_int_fp_prune_simple.count()
            << std::endl;
  // auto* lpfHeuristic_bt = new BV_BlockTree_lpf_heuristic<uint8_t,
  // int32_t>(vec, tau, mls);
  auto t06 = std::chrono::high_resolution_clock::now();
  auto ms_int_lpf_heu =
      std::chrono::duration_cast<std::chrono::milliseconds>(t06 - t0y);
  std::cout << "LPF Heuristic TIME " << ms_int_lpf_heu.count() << std::endl;
  // auto* lpfTheory_bt = new BV_BlockTree_lpf_theory<uint8_t, int32_t>(vec,
  // tau, mls, false);
  auto t07 = std::chrono::high_resolution_clock::now();
  auto ms_int_lpf_theo =
      std::chrono::duration_cast<std::chrono::milliseconds>(t07 - t06);
  std::cout << "LPF THEORY TIME " << ms_int_lpf_theo.count() << std::endl;
  // auto* lpfTheory_bt_dp = new BV_BlockTree_lpf_theory<uint8_t, int32_t>(vec,
  // tau, mls, true);
  auto t0aa = std::chrono::high_resolution_clock::now();
  auto ms_int_lpf_theo_dp =
      std::chrono::duration_cast<std::chrono::milliseconds>(t0aa - t07);
  std::cout << "LPF THEORY DP TIME " << ms_int_lpf_theo_dp.count() << std::endl;
  auto *lpfPruned_bt = new BlockTreeLPF<uint8_t, int32_t>(
      vec, tau, mls, true, CUT_FIRST_LEVELS, true);
  auto t08 = std::chrono::high_resolution_clock::now();
  auto ms_int_lpf_prune =
      std::chrono::duration_cast<std::chrono::milliseconds>(t08 - t0aa);
  std::cout << "LPF PRUNED TIME " << ms_int_lpf_prune.count() << std::endl;
  auto *lpfPruned_bt_dp = new BlockTreeLPF<uint8_t, int32_t>(
      vec, tau, mls, true, CUT_FIRST_LEVELS, false);
  auto t09 = std::chrono::high_resolution_clock::now();
  auto ms_int_lpf_prune_dp =
      std::chrono::duration_cast<std::chrono::milliseconds>(t09 - t08);
  std::cout << "LPF PRUNED DP TIME " << ms_int_lpf_prune_dp.count()
            << std::endl;
  std::cout << "time " << ms_int.count() << std::endl;
  // std::cout << "fp theory space " << fpTheory_bt->print_space_usage() << " "
  // << fpTheory_bt->block_tree_types_.size() <<  std::endl;
  std::cout << "fp pruned space " << fpPruned_bt->print_space_usage() << " "
            << fpPruned_bt->block_tree_types_.size() << std::endl;
  std::cout << "fp pruned simple space "
            << fpPruned_simple_bt->print_space_usage() << " "
            << fpPruned_bt->block_tree_types_.size() << std::endl;
  // std::cout << "lpfArray heuristic space " <<
  // lpfHeuristic_bt->print_space_usage() << " " <<
  // lpfHeuristic_bt->block_tree_types_.size() <<   std::endl; std::cout <<
  // "lpfArray theory space " << lpfTheory_bt->print_space_usage() << " " <<
  // lpfTheory_bt->block_tree_types_.size() <<   std::endl; std::cout <<
  // "lpfArray theory dp space " << lpfTheory_bt_dp->print_space_usage() << " "
  // << lpfTheory_bt_dp->block_tree_types_.size() <<   std::endl;
  std::cout << "lpfArray pruned space " << lpfPruned_bt->print_space_usage()
            << " " << lpfPruned_bt->block_tree_types_.size() << std::endl;
  std::cout << "lpfArray pruned dp space "
            << lpfPruned_bt_dp->print_space_usage() << " "
            << lpfPruned_bt_dp->block_tree_types_.size() << std::endl;
  //    BV_BlockTree_lpf_pruned<uint8_t, int32_t>*  lpf_bt2 = new
  //    BV_BlockTree_lpf_pruned<uint8_t, int32_t>(vec, 2, 1, 15,lpfArray,
  //    prevOcc, lzPhrases, false);
  std::cout << "lpfArray" << std::endl;
  //    BV_BlockTree_lpf_pruned<uint8_t, int32_t>*  lpf_bt = new
  //    BV_BlockTree_lpf_pruned<uint8_t, int32_t>(vec, 2, 1, 15,lpfArray,
  //    prevOcc, lzPhrases, true);
  ...
  //        std::cout << "(" << lpf_bt->block_tree_pointers_[i]->size() << ","
  //        << (int) lpf_bt->block_tree_pointers_[i]->width() << "," <<
  //        lpf_bt->block_tree_pointers_[i]->bit_size() << ")/"; std::cout <<
  //        "(" << lpf_bt->block_tree_offsets_[i]->size() << "," << (int)
  //        lpf_bt->block_tree_offsets_[i]->width() << "," <<
  //        lpf_bt->block_tree_offsets_[i]->bit_size() << ")" << std::endl;
  //    }
  ...
  //        std::cout << "(" << lpf_bt2->block_tree_pointers_[i]->size() << ","
  //        << (int) lpf_bt2->block_tree_pointers_[i]->width() << "," <<
  //        lpf_bt2->block_tree_pointers_[i]->bit_size() << ")/"; std::cout <<
  //        "(" << lpf_bt2->block_tree_offsets_[i]->size() << "," << (int)
  //        lpf_bt2->block_tree_offsets_[i]->width() << "," <<
  //        lpf_bt2->block_tree_offsets_[i]->bit_size() << ")" << std::endl;
  //    }
  //
  ...
  //        std::cout << "(" << fp_bt->block_tree_pointers_[i]->size() << "," <<
  //        (int) fp_bt->block_tree_pointers_[i]->width() << "," <<
  //        fp_bt->block_tree_pointers_[i]->bit_size() << ")/"; std::cout << "("
  //        << fp_bt->block_tree_offsets_[i]->size() << "," << (int)
  //        fp_bt->block_tree_offsets_[i]->width() << "," <<
  //        fp_bt->block_tree_offsets_[i]->bit_size() << ")" << std::endl;
  //    }
  int j = 0;
  ...
  auto ms_int_rs =
      std::chrono::duration_cast<std::chrono::milliseconds>(t0r2 - t0r1);
  ...
  auto ms_int_rs2 =
      std::chrono::duration_cast<std::chrono::milliseconds>(t0r4 - t0r3);
  std::cout << "RS TIME " << 1 << ": " << ms_int_rs2.count() << std::endl;

  //    for (auto c: lpf_bt->chars_) {
  //        std::cout << c << ":";
  //        for (int i = 0; i <
  //        lpf_bt->c_ranks_[lpf_bt->chars_index_[c]].size(); i++) {
  //            std::cout << (int)
  //            lpf_bt->c_ranks_[lpf_bt->chars_index_[c]][i].width() << "/" <<
  //            (int) lpf_bt->c_ranks_[lpf_bt->chars_index_[c]][i].size() << "
  //            ";
  //        }
  //        std::cout << std::endl;
  //    }
  ...
  //        std::vector<int>(lpf_bt->block_tree_types_[i]->size(), 2); for (int
  //        k = 0; k < lpf_bt->block_tree_types_[i]->size(); k++) {
  ...
  //        fp_bt->block_tree_pointers_[i]->size(); j++)
  //        {
  //            if ((*fp_bt->block_tree_pointers_[i])[j] !=
  //            (*lpf_bt->block_tree_pointers_[i])[j]) {
  //                std::cout << i << " " << j << " " <<
  //                (*fp_bt->block_tree_pointers_[i])[j] << " " <<
  //                (*lpf_bt->block_tree_pointers_[i])[j] << std::endl;
  //            }
  //        }
  //    }
  //    for (int i = 0; i < fp_bt->block_tree_types_.size(); i++) {
  //        for (int j = 0; j < lpf_bt->block_tree_offsets_[i]->size() && j <
  //        fp_bt->block_tree_offsets_[i]->size(); j++)
  //        {
  //            if ((*fp_bt->block_tree_offsets_[i])[j] !=
  //            (*lpf_bt->block_tree_offsets_[i])[j]) {
  //                std::cout << i << " " << j << " " <<
  //                (*fp_bt->block_tree_offsets_[i])[j] << " " <<
  //                (*lpf_bt->block_tree_offsets_[i])[j] << std::endl;
  //            }
  //        }
  //    }
  ...
  //        if ((*lpf_bt->block_tree_pointers_[1])[i] % lpf_bt->tau_ ==
  //        lpf_bt->tau_ - 1) {
  //            std::cout << "warum " << i <<
  //            (*lpf_bt->block_tree_pointers_[1])[i] <<  std::endl;
  //        }
  //    }
  //    for (auto c: lpf_bt->chars_) {
  ...
  //            if (i % 2 == 1) {
  //                if (lpf_bt->c_ranks_[d][1][i - 1] >
  //                lpf_bt->c_ranks_[d][1][i]) {
  ...
  //    {206,207,268,269,772,773,802,803,830,831,1014,1015,1022,1023}; for (auto
  //    q: error) {
  ...
  //            if ((bool)(*lpf_bt->block_tree_types_[i])[j] !=
  //            (bool)(*fp_bt->block_tree_types_[i])[j]) {
  //                std::cout << i << " " << j << " lpfArray " <<
  //                (*lpf_bt->block_tree_types_[i])[j] << " fp " <<
  //                (*fp_bt->block_tree_types_[i])[j] << std::endl;
  //            }
  //        }
  //    }
  ...
  auto elapsed3 = std::chrono::duration_cast<std::chrono::nanoseconds>(
  ...
  auto elapsed2 = std::chrono::duration_cast<std::chrono::nanoseconds>(
  ...
  auto elapsed4 = std::chrono::duration_cast<std::chrono::nanoseconds>(
  ...
  auto elapsed5 = std::chrono::duration_cast<std::chrono::nanoseconds>(
                      std::chrono::high_resolution_clock::now() - start5)
                      .count();
  for (uint64_t i = 0; i < select_c_.size(); i++) {
    if (lpfPruned_bt->rank_base(select_c_[i], access_queries_[i]) !=
        lpfPruned_bt->rank(select_c_[i], access_queries_[i]))
      std::cout << i << std::endl;
  }
  double nanosec_per_access =
      elapsed / static_cast<double>(access_queries_.size());
  double nanosec_per_access2 =
      elapsed3 / static_cast<double>(access_queries_.size());
  double nanosec_per_rank =
      elapsed2 / static_cast<double>(access_queries_.size());
  double nanosec_per_rank2 =
      elapsed4 / static_cast<double>(access_queries_.size());
  double nanosec_per_select =
      elapsed5 / static_cast<double>(access_queries_.size());
  std::cout << "#access queries " << access_queries_.size() << std::endl;
  std::cout << nanosec_per_access << " ns per query\n";
  std::cout << "#access encoded queries " << access_queries_.size()
            << std::endl;
